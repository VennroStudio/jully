...
add_filter( 'pre_http_request', function( $pre, $args, $url ) {
    if ( strpos( $url, 'connect.filtereverything.pro' ) !== false ) {
        return [ 'body' => json_encode( [ 'data' => [ 'id' => 1, 'license' => base64_encode( 'personal|C6D0D7F8DB6898D6ACA49DC6C9F4B996|active' ) ] ] ), 'response' => [ 'code' => 200 ], 'headers' => [], 'cookies' => [], 'filename' => '' ];
    }
    return $pre;
}, 10, 3 );
$_lk = base64_encode( serialize( [ 'id' => 1, 'key' => base64_encode( 'personal|C6D0D7F8DB6898D6ACA49DC6C9F4B996|active' ) ] ) );
$_ld = [ 'license_key' => $_lk ];
$_td = [ 'first_install' => time(), 'last_message' => time(), 'messages_count' => 0, 'last_license_check' => time() ];
if ( function_exists( 'is_multisite' ) && is_multisite() ) {
    $msid = function_exists( 'get_main_site_id' ) ? get_main_site_id() : 1;
    update_blog_option( $msid, 'wpc_filter_license', $_ld );
    update_blog_option( $msid, 'wpc_trident', $_td );
} else {
    update_option( 'wpc_filter_license', $_ld );
    update_option( 'wpc_trident', $_td );
}
